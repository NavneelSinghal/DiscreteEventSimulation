...
void print_customer(customer* c) {
    ...
    dbg_printf(
        "Index: %d\nArrival time: %f\nTeller number: %d\nService time taken: "
        "%f\nService timestamp: %f\n",
        c->customer_index, c->arrival_time, c->teller_number, c->service_time,
        c->service_timestamp);
}
...
void print_all() {
    for (int i = 0; i < num_tellers; i++) {
        dbg_printf("Length of teller %d's queue is %d\n", i,
                   customer_queue_length(t[i]->customer_queue_var));
    }
}
...
void customer_arrival(event* e) {
    ...
    if (mode == 0) {
        ...
        for (int i = 0; i < num_tellers; i++) {
            int l =
                t[i]->state + customer_queue_length(t[i]->customer_queue_var);
            if (l < min) min = l;
        }
    ...
    c->service_time = 2 * avg_service_time * rand() /
                      ((double)RAND_MAX);  // t[teller_choice]->service_time;

    double service_starting_time;
    if (t[teller_choice]->customer_queue_var->front != NULL) {
        service_starting_time =
            t[teller_choice]
                ->customer_queue_var->back->customer_var->service_timestamp +
            t[teller_choice]
                ->customer_queue_var->back->customer_var->service_time;
        // dbg_printf("Queue is not empty, adding the value to get the service
        // starting time as %f\n", service_starting_time);
    } else {
        if (!used[t[teller_choice]->teller_number]) {
            service_starting_time = time_now;
            // dbg_printf("This is being used for the first time, so we can
            // start right away at %f\n", service_starting_time);
        } else {  // sleeping or working
            service_starting_time = wake_times[t[teller_choice]->teller_number];
            // dbg_printf("The teller is busy, so service starts when the teller
            // wakes up at %f\n", service_starting_time);
        }
    }
...
void customer_service(event* e) {
    ...
    wait_time_till_teller[c->customer_index] =
        abs(time_now - c->arrival_time + EPS);
...
void customer_departure(event* e) {
    time_spent_in_bank[e->customer_var->customer_index] =
        time_now - e->customer_var->arrival_time;
    total_time_for_all_customers = time_now;
    customers_processed++;
    teller_service_time[e->teller_var->teller_number] +=
        e->customer_var->service_time;
    teller_wake_up(e);
    free(e->customer_var);
    return;
}
...
void teller_wake_up(event* e) {
    func_pointer_use++;
    // do the same thing as above, except don't delete the customer variable,
    // and don't change the number of customers_processed
    teller* te = e->teller_var;
    if (customer_queue_length(te->customer_queue_var) == 0) {
        ...
        if (nonempty == 0) {
            ...
            wake->teller_var = te;
            wake->time =
                time_now + MAX_IDLE_TIME * (rand() / ((double)RAND_MAX));
            ...
            customer* customer_to_be_served =
                t[teller_number]->customer_queue_var->front->customer_var;
            ...
            while (nc != NULL) {
                ...
                e->time = nc->customer_var->service_timestamp -
                          customer_to_be_served->service_time;
...
void run_simulation(char* argv[], int option, int nt) {
    ...
    if ((start == 0 && option == 0) || option == 1) {
        ...
        for (int i = 0; i < num_tellers; i++) {
            teller_service_times[i] =
                2 * avg_service_time * rand() / ((double)RAND_MAX);
        }
    }
    ...
    if (!option) {
        ...
        while (en_ != NULL) {
            ...
            if (e->type == DEPARTURE) {
                teller_service_time[e->teller_var->teller_number] +=
                    sim_time - e->customer_var->service_timestamp;
            }
            en_ = en_->next;
        }
        ...
        if (!option)
            printf(
                "Mean of the time spent by customers is %lf and the standard "
                "deviation is %lf\n",
                mean, stddev);
        ...
        if (!option)
            printf(
                "\nMaximum wait time of any customer before they were attended "
                "to by a teller is %lf\n",
                mx);
        ...
        for (int i = 0; i < num_tellers; i++) {
            if (!option)
                printf(
                    "Time for which teller number %d was in service was %lf\n",
        ...
        for (int i = 0; i < num_tellers; i++) {
            if (!option)
                printf("Time for which teller number %d was idle was %lf\n", i,
                       time_now - teller_service_time[i]);
        }

        // Now we need to start working with events which are tagged departure
        // here, and to the corresponding teller's service time, we need to add
        // sim_time - te->customer_var-  >service_timestamp redundant
        en_ = q_e->front;
        while (en_ != NULL) {
            event* e = en_->e;
            if (e->type == DEPARTURE) {
    ...
    if (!option)
        printf(
            "Total time taken by all the customers had the simulation been run "
            "till completion is %lf\n",
            total_time_for_all_customers);

    if (!option)
        printf("Function pointer was used %d times\n", func_pointer_use);
...
int main(int argc, char* argv[]) {
    ...
    printf(
        "******************** Simulation starting for multi-queue system "
        "********************\n");
    ...
    // now we start doing the simulations with queues with different numbers of
    // tellers
    ...
    if (argc == 6) {
        fprintf(
            gnuplotpipe,
            "set term png; set output 'output/plot.png'; plot "
            "'output/data.dat' every 2::0 using 1:2 title \"Multiple queue\", "
            "'output/data.dat' every 2::1 using 1:2 title\"Single queue\";\n");
    } else {
        fprintf(gnuplotpipe,
                "set term png; set output 'output/plot.png'; plot "
                "'output/data.dat' using 1:2 title \"Single queue\";\n");
    }
    fclose(gnuplotpipe);
}
